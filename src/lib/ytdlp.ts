...
export async function commonYtdlpArgs(): Promise<string[]> {
  ...
  if (bgutilUrl) {
    // Fetch bgutil tokens and (if cookies are available) the Data Sync ID in
    // parallel to avoid adding latency.
    const [tokens, dataSyncId] = await Promise.all([
    ...
    if (tokens?.visitorData) {
      ...
      // Build extractor args. The po_token and visitor_data MUST be passed
      // together — they are a matched pair generated by bgutil. Passing one
      // without the other causes YouTube to serve only the 360p legacy stream.
      //
      // player_skip=webpage,configs prevents the page visit from replacing our
      // visitor_data. player.js is still downloaded (no 'js' in player_skip)
      // so n-challenge + signature solving continue to work.
      //
      // data_sync_id identifies the signed-in account to the player API.
      // It is the DELEGATED_SESSION_ID value from the YouTube homepage, fetched
      // above using the cookies. Without it, YouTube omits adaptive formats for
      // authenticated sessions when player_skip is active.
      const ytArgs = [
        'player_client=web',
      ...
      if (dataSyncId) ytArgs.push(`data_sync_id=${dataSyncId}`)
